...
public:
  ...
  friend int2048 div(int2048, const int2048);
  ...
  int tota, sym, sta;//sta = 0: int, sta = 1: double
  std :: vector < int > a;
};
} // namespace sjtu
...
sjtu :: int2048 :: int2048 (long long x)
{
  ...
  if (tota <= 0) {tota = 1; a.push_back(0);}
  sta = 0;
}
sjtu :: int2048 :: int2048 (const std::string &s)
{
  tota = 0; int i, j, m = s.length(), beg;
  sta = 0;
  for (i = 0; i < m; i++) if (s[i] == '.') {sta = 1; break;}
  if (a.size()) {for (i = 0; i < a.size(); i++) a[i] = 0;}
  if (sta)
  {
    for (j = i + 1; j <= std::min(i + 6, m); j++)
      a[0] = (a[0] << 1) + (a[0] << 3) + (s[j] & 15);
    if (m < i + 6) for (j = m + 1; j <= i + 6; j++)
      a[0] = (a[0] << 1) + (a[0] << 3);
    m = i;
  }
  sym = (1 - ((s[0] == '-') << 1)); beg = (s[0] == '-');
...
sjtu :: int2048 :: int2048 (const sjtu :: int2048 &x)
{
  int i;
  if (a.size()) {for (i = 0; i < a.size(); i++) a[i] = 0;}
  tota = x.tota; sym = x.sym; sta = x.sta;
  for (i = 0; i <= tota; i++) 
...
{
  sjtu :: int2048 x = xx;
  int i, j, m = std :: max(tota, x.tota), seg = sym * x.sym, sta = (sta | x.sta), com = 0;
...
{
//  printf ("%lld\n", qpow(g, (mod - 2)));
  int i, j, m = std :: max(y.tota, x.tota), lim = 1, lglim = 0;
...
//	for (i = 0; i <= y.tota; i++) printf("%lld ", Ap[i]); printf("\n");
  ...
  for (i = 0; i <= y.tota - 1; i++) y.a[i] = y.a[i + 1];
  if (y.tota > 1) y.tota--;
...
sjtu :: int2048 sjtu :: div (sjtu :: int2048 y, const sjtu :: int2048 xx)
{
  int i, j, tmp;
  y *= (int2048)((long long)Amodd);
  sjtu :: int2048 x = xx, yy;
  if (x.sym == y.sym) {x.sym = 1; y.sym = 1;}
  else {x.sym = -1; y.sym = 1;}
  tmp = x.sym;
  x.sym = 1;
